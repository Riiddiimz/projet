...
function addChatMessage(message){
    ...
    const isRoomMessage = !!message.roomId;

    if(isRoomMessage){
        const isMine = currentUser && message.userId === currentUser.id;
        const chatOpen = roomChat && roomChat.classList.contains("mobile-open");

        appendChatMessage(
            document.getElementById("roomChatMessages"),
            message
        );
...
function updateRoomUnreadBadge(){
    const count = roomUnreadCount;
    const badges = [
        document.getElementById("roomChatUnread"),
        document.getElementById("desktopRoomChatUnread")
    ];
...
function closeGeneralChat(){
    ...
    if(mobileChatBackdrop){
        mobileChatBackdrop.classList.remove("visible");
    }
}
...
function toggleUsersSidebar(){
    ...
    if(button){
        ...
        button.setAttribute(
            "aria-label",
            hidden ? "Afficher les utilisateurs" : "Masquer les utilisateurs"
        );
    }
}
...
function toggleRoomChat(){
    ...
    const isOpen = roomChat.classList.contains("mobile-open");
    closeMobileChat();

    if(!isOpen){
        roomChat.classList.add("mobile-open");
...
function closeRoomChat(){
    if(roomChat){
        roomChat.classList.remove("mobile-open");
    }
...
const lobbyChatInputElement = document.getElementById("lobbyChatInput");

if(lobbyChatInputElement){
    lobbyChatInputElement.addEventListener("keydown", event => {
        if(event.key === "Enter"){
...
const roomChatInputElement = document.getElementById("roomChatInput");

if(roomChatInputElement){
    roomChatInputElement.addEventListener("keydown", event => {
        if(event.key === "Enter"){
            event.preventDefault();
            sendRoomChat();
        }
