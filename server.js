...
function sendRoomList() { broadcast({ type: "room-list", rooms: [...rooms.values()].map(publicRoom) }); }
function sendUserList() { broadcast({ type: "user-list", users: [...users.values()].map(publicUser) }); broadcast({ type: "online-user-list", users: [...users.values()].filter(isOnline).map(publicUser) }); }
