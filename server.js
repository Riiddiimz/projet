...
function removeUserFromRoom(user, notify = true) {
  ...
  if (notify) broadcastRoom(roomId, { type: "room-user-left", userId: user.id });
  if (!room.permanent && room.users.size === 0) rooms.delete(roomId);
  sendRoomList();
  sendUserList();
}
...
function findUserByUsername(username) {
  ...
  for (const user of users.values()) if (user.username.toLowerCase() === normalized) return user;
  return null;
}
...
function chatMessage(user, text, roomId) {
  return { type: "chat", id: nextMessageId++, userId: user.id, username: user.username, text, roomId, timestamp: Date.now() };
}
...
wss.on("connection", ws => {
  ...
  ws.on("message", raw => {
    ...
    if (type === "restore-session") {
      ...
      if (!user) { sessions.delete(token); safeSend(ws, { type: "session-invalid" }); return; }
    ...
    if (type === "login") {
      ...
      if (existing && isOnline(existing)) { safeSend(ws, { type: "login-error", message: "Ce nom d'utilisateur est déjà utilisé." }); return; }
      const user = existing || { id: `user-${nextUserId++}`, username, isAdmin, ws: null, roomId: null, microphoneEnabled: false, cameraEnabled: false, description: "", avatarUrl: null, connectedAt: Date.now(), lastSeenAt: Date.now() };
    ...
    if (type === "update-profile") {
      ...
      if (typeof message.avatarUrl === "string" && message.avatarUrl.length <= MAX_AVATAR_LENGTH) currentUser.avatarUrl = message.avatarUrl;
      else if (message.avatarUrl === null) currentUser.avatarUrl = null;
    ...
    if (type === "create-room") {
      ...
      const room = { id: `room-${nextRoomId++}`, name, ownerId: currentUser.id, users: new Set(), createdAt: Date.now(), permanent: false };
    ...
    if (type === "join-room") {
      ...
      broadcastRoom(roomId, { type: "room-user-joined", user: publicUser(currentUser) }, currentUser.id);
      sendRoomList();
      sendUserList();
      return;
    }
    ...
    if (type === "chat") {
      ...
      const payload = chatMessage(currentUser, text, roomId);
      if (roomId) broadcastRoom(roomId, payload);
      else broadcast(payload, user => isOnline(user) && !user.roomId);
      return;
    }
    ...
    if (type === "offer" || type === "answer" || type === "candidate") {
      ...
      const sourceRoomId = currentUser.roomId;
      if (!sourceRoomId || !target || !isOnline(target) || target.roomId !== sourceRoomId || target.id === currentUser.id) {
        safeSend(ws, { type: "signaling-error", message: "Pair WebRTC invalide." });
        return;
      }
    ...
    if (type === "media-state") {
      if (typeof message.microphoneEnabled === "boolean") currentUser.microphoneEnabled = message.microphoneEnabled;
      if (typeof message.cameraEnabled === "boolean") currentUser.cameraEnabled = message.cameraEnabled;
      if (currentUser.roomId) broadcastRoom(currentUser.roomId, { type: "remote-media-state", userId: currentUser.id, microphoneEnabled: currentUser.microphoneEnabled, cameraEnabled: currentUser.cameraEnabled }, currentUser.id);
      sendUserList();
      return;
    }
    ...
    if (type === "admin-mute" || type === "admin-camera") {
      const target = users.get(String(message.userId || ""));
      if (!target) return;
      if (type === "admin-mute") target.microphoneEnabled = false;
      else target.cameraEnabled = false;
      safeSend(target.ws, { type: "force-media-state", microphoneEnabled: target.microphoneEnabled, cameraEnabled: target.cameraEnabled, reason: type });
      if (target.roomId) broadcastRoom(target.roomId, { type: "remote-media-state", userId: target.id, microphoneEnabled: target.microphoneEnabled, cameraEnabled: target.cameraEnabled }, target.id);
      sendUserList();
      return;
    }
    ...
    if (type === "admin-kick") {
      ...
      target.ws = null;
      safeSend(targetWs, { type: "force-lobby", message: "Vous avez été exclu par l'administrateur." });
...
setInterval(() => {
  ...
  for (const [token, session] of sessions) {
    ...
    const user = users.get(session.userId);
    if (user && !isOnline(user)) users.delete(user.id);
  }
  sendRoomList();
  sendUserList();
}, 60 * 60 * 1000);
