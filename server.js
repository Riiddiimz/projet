...
wss.on("connection", ws => {
  ...
  ws.on("close", () => { if (!currentUser || currentUser.ws !== ws) return; const user = currentUser; user.ws = null; user.lastSeenAt = Date.now(); user.microphoneEnabled = false; user.cameraEnabled = false; user.diagnosticOptIn = false; if (user.roomId) removeUserFromRoom(user, true); sendRoomList(); sendUserList(); });
});
