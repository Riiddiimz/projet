...
async function snapshot(page, label) {
  const data = {
    ...
    authScreen: await inspectElement(page,'#authScreen'), app: await inspectElement(page,'#app'), lobby: await inspectElement(page,'#lobbyScreen'),
    usernameInput: await inspectElement(page,'#usernameInput'), passwordInput: await inspectElement(page,'#passwordInput'),
    usersButton: await inspectElement(page,'#usersSidebarToggle'), usersSidebar: await inspectElement(page,'#lobbyScreen .users-sidebar'),
    generalChatButton: await inspectElement(page,'#generalChatToggle'), roomList: await inspectElement(page,'#roomList'), roomScreen: await inspectElement(page,'#roomScreen')
  };
...
async function login(page) {
  ...
  page.on('response', response => {
    ...
    if (url === SITE_URL || url.startsWith(SITE_URL)) responseInfo = { url, status:response.status(), contentType:response.headers()['content-type']||'' };
  });
  try { await page.goto(SITE_URL,{waitUntil:'domcontentloaded',timeout:60000}); }
  catch(error) { await page.screenshot({path:`${OUT}/00-goto-error.png`,fullPage:true}).catch(()=>{}); record('Login form','FAIL',{reason:'page.goto failed',error:error.message,url:page.url()}); return false; }
  await page.waitForLoadState('networkidle',{timeout:10000}).catch(()=>{});
  await page.waitForTimeout(2000);

  // The deployed site may use a two-step auth screen: first click "Utilisateur", then the form appears.
  let username = page.locator('#usernameInput').first();
  if (!(await username.count())) {
    const userButton = page.getByRole('button',{name:/Utilisateur/i}).first();
    if (await userButton.count()) {
      await userButton.click().catch(()=>{});
      await page.waitForTimeout(500);
      username = page.locator('#usernameInput').first();
    }
  }
  ...
  if (!(await username.count())) {
    ...
    record('Login form','FAIL',{reason:'usernameInput missing after opening user login',url:page.url(),title:initial.title,readyState:initial.readyState,response:responseInfo,bodyPreview:initial.bodyText.slice(0,1200),htmlHasAuthScreen:html.includes('authScreen'),htmlHasUsernameInput:html.includes('usernameInput'),buttons:await page.locator('button').allTextContents().catch(()=>[])});
    return false;
  }
  ...
  if (!TEST_PASSWORD) { record('Authentication','BLOCKED',{reason:'TEST_PASSWORD secret is not configured',username:TEST_USERNAME}); return false; }

  const loginButton = page.locator('button').filter({hasText:/Se connecter|Connexion|Entrer/i}).first();
  if (!(await loginButton.count())) { record('Authentication','FAIL',{reason:'login button missing',buttons:await page.locator('button').allTextContents().catch(()=>[])}); return false; }
  await loginButton.click();
  try { await page.locator('#lobbyScreen').waitFor({state:'visible',timeout:15000}); record('Authentication','PASS',{username:TEST_USERNAME}); await page.screenshot({path:`${OUT}/02-lobby-after-login.png`,fullPage:true}); return true; }
  catch { const error=await page.locator('#loginError').textContent().catch(()=>'' ); record('Authentication','FAIL',{username:TEST_USERNAME,error:(error||'').trim(),url:page.url()}); return false; }
}
...
async function testUsersSidebar(page) {
  ...
  const before=await snapshot(page,'03-before-users'); await button.click({force:true}); await page.waitForTimeout(500); const after=await snapshot(page,'04-after-users');
  await page.screenshot({path:`${OUT}/04-after-users.png`,fullPage:true});
  const changed=JSON.stringify(before.usersSidebar)!==JSON.stringify(after.usersSidebar)||JSON.stringify(before.lobby)!==JSON.stringify(after.lobby)||before.usersButton.text!==after.usersButton.text;
  record('Lobby / Utilisateurs',changed?'PASS':'FAIL',{buttonBefore:before.usersButton,buttonAfter:after.usersButton,sidebarBefore:before.usersSidebar,sidebarAfter:after.usersSidebar,lobbyAfter:after.lobby});
  await button.click({force:true}); await page.waitForTimeout(300); record('Lobby / Utilisateurs / toggle retour','PASS',await inspectElement(page,'#lobbyScreen .users-sidebar'));
}
...
async function testLobby(page) {
  ...
  const search=page.locator('#roomSearch'); if(await search.count()){ await search.fill('TEST'); await page.waitForTimeout(300); record('Lobby / recherche TEST','PASS',{value:await search.inputValue()}); await search.fill(''); }
}
...
async function runDevice(name,device){
  ...
  if(authenticated){await testLobby(page);await testUsersSidebar(page);await testButtons(page);await snapshot(page,`05-final-${name}`);}
