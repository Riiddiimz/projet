...
async function layout(page,label){return page.evaluate(l=>{const all=[...document.querySelectorAll('body *')];const vw=innerWidth,vh=innerHeight;const overflow=all.filter(e=>{const r=e.getBoundingClientRect();return r.width>0&&(r.right>vw+2||r.left<-2||r.bottom>vh+2)}).slice(0,15).map(e=>({tag:e.tagName,id:e.id,cls:e.className?.toString().slice(0,80)}));return {label:l,viewport:[vw,vh],bodyScroll:[document.body.scrollWidth,document.body.scrollHeight],viewportOverflow:overflow};},label);}
...
(async()=>{
 ...
 try{
  ...
  const mobile=await context.newPage();await mobile.setViewportSize({width:390,height:844});await mobile.context().grantPermissions(['camera','microphone']); const mobileErrors=[];mobile.on('pageerror',e=>mobileErrors.push(e.message));await login(mobile,`${USER}_M`);await screenshot(mobile,'02-lobby-mobile');
  ...
  const create=await button(mobile,'Créer un salon');if(!create)throw Error('Création salon introuvable');
  mobile.once('dialog',async dialog=>{await dialog.accept(ROOM);});
  await create.click();
  await mobile.waitForFunction(n=>(document.querySelector('#roomList')?.innerText||'').includes(n),ROOM,{timeout:20000});
  await screenshot(mobile,'04-room-list-mobile');record('UX / création salon depuis mobile','PASS');
